...
| Возможность | NGINX | Kong | Traefik | Envoy | Spring Cloud Gateway |
|-------------|-------|------|---------|-------|---------------------|
| **Маршрутизация на основе конфигурации** | ✅ (файлы .conf) | ✅ (админ API / декларативная) | ✅ (автоматическая из Docker/K8s) | ✅ (файлы .yaml / xDS) | ✅ (Java DSL / YAML) |
| **Проверка аутентификации** | ⚠️ (через lua-скрипты или модули) | ✅ (JWT, Key-Auth, OAuth2, LDAP) | ✅ (JWT, Forward Auth) | ⚠️ (через external auth filter) | ✅ (Spring Security) |
| **Терминация HTTPS** | ✅ | ✅ | ✅ | ✅ | ✅ |
| **Лимитирование запросов** | ✅ (через модули) | ✅ (встроенный rate-limiting) | ✅ (middleware) | ✅ | ✅ |
| **Логирование и мониторинг** | ✅ (access_log, metrics module) | ✅ (Prometheus, StatsD) | ✅ (Prometheus, Tracing) | ✅ (отличные метрики) | ✅ (Micrometer) |
| **Динамическое обновление конфига** | ❌ (требует reload) | ✅ (через API) | ✅ (автоматически) | ✅ (через xDS) | ⚠️ (требует перезапуска) |
| **Простота настройки** | ⭐⭐⭐⭐ | ⭐⭐⭐ | ⭐⭐⭐⭐⭐ | ⭐⭐ | ⭐⭐⭐ |
| **Производительность** | ⭐⭐⭐⭐⭐ | ⭐⭐⭐⭐ | ⭐⭐⭐⭐ | ⭐⭐⭐⭐⭐ | ⭐⭐⭐ |
| **Поддержка gRPC** | ⚠️ (ограниченная) | ✅ | ✅ | ✅ | ✅ |
| **Service Discovery** | ⚠️ (через DNS или сторонние модули) | ✅ (Consul, K8s) | ✅ (встроенная) | ✅ (через xDS) | ✅ (Eureka, K8s) |
